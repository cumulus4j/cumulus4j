...
//	}
	...
	public Ciphertext encrypt(CryptoContext cryptoContext, Plaintext plaintext)
	{
		...
		try {
			...
			if (!CryptoManager.MAC_ALGORITHM_NONE.equals(encryptionCoordinateSet.getMACAlgorithm())) {
				...
				mac = macCalculator.doFinal(plaintext.getData().getBytes());
			...
			int outLength = (
					...
					+ cipher.getOutputSize(
							...
							+ plaintext.getData().getBytes().length // actual plaintext
			...
			outOff += cipher.update(plaintext.getData().getBytes(), 0, plaintext.getData().getBytes().length, out, outOff);
			...
			ciphertext.setData(new Blob(out));
	...
	public Plaintext decrypt(CryptoContext cryptoContext, Ciphertext ciphertext)
	{
		...
		try {
			...
			int inOff = 0;
			byte[] in = ciphertext.getData().getBytes();
			...
			plaintext.setData(new Blob(decrypted));
			return plaintext;
		} catch (RuntimeException e) {
