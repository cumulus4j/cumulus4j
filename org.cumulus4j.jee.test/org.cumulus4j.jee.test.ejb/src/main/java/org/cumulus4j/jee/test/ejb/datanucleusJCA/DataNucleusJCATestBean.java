...
public class DataNucleusJCATestBean extends AbstractDataNucleusJCATestBean implements
		DataNucleusJCATestRemote {

	private static final Logger logger = LoggerFactory
			.getLogger(DataNucleusJCATestBean.class);

	@EJB
	private DataNucleusJCANewTransactionBean dataNucleusJCANewTransactionBean;

	@EJB
	private DataNucleusJCASharedTransactionBean dataNucleusJCASharedTransactionBean;
	...
	private void testRollback(UUID id1, UUID id2,
		...
		try {
			if (nestedTransaction)
				dataNucleusJCANewTransactionBean.testRollback(id2,
						throwExceptionInNestedBeanCall);
			else
				dataNucleusJCASharedTransactionBean.testRollback(id2,
						throwExceptionInNestedBeanCall);
		} catch (Exception x) {
